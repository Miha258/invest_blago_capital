...
func (s *AuthService) GenerateRefreshToken(user models.SingIn) (string, error) {
	refreshToken, err := bcrypt.GenerateFromPassword([]byte(user.Login), 8)

	if err != nil {
		return "", errors.New("An error occurred generating refresh token")
	}
