...
func (h *Handler) updateAccountDocs(c *gin.Context) {
	...
	if err := h.services.EditUserData(userId.(int), input); err != nil {
		logrus.Error(err)
		newErrorResponse(c, http.StatusBadRequest, err.Error())
		return
	}
...
func (h *Handler) updateAccountBalance(c *gin.Context) {
	...
	userId, _ := c.Get(userCtx)
	if err := h.services.UpdateBalance(userId.(int), input.Sum); err != nil {
		logrus.Error(err)
		newErrorResponse(c, http.StatusBadRequest, err.Error())
		return
	}
...
func (h *Handler) createPromocode(c *gin.Context) {
	...
	if err := h.services.CreatePromocode(userId.(int), input.Name); err != nil {
		logrus.Error(err)
		newErrorResponse(c, http.StatusBadRequest, err.Error())
		return
	}
